...
onMounted(async () => {
  ...
  await nextTick()
  // 初始加载由 van-list 的 load 事件触发，这里不需要手动调用 loadMore
})
  ...
  try {
    ...
    if (res.code === '200') {
      ...
      if (page.value === 1) {
        ...
        sellList.value = [...sellList.value, ...list]
      }
      
      // 判断是否加载完成
      if (list.length < pageSize.value || page.value >= (res.data.totalPage || 1)) {
        finished.value = true
      }
      else {
        page.value++
...
function checkSellState() {
  router.push('/sell/stats')
}
</script>

<template>
  <div class="bg-[#FFF9F5] min-h-screen pb-[50px]">
  
    <van-pull-refresh v-model="refreshing" class="min-h-[calc(100vh-100px)]" @refresh="onRefresh">
      <!-- 内容 -->
      <div class="mx-auto px-3 py-3 max-w-[480px]">
        <!-- 顶部标签 -->
        <div class="mb-3 p-1 rounded-2xl bg-white flex shadow-sm items-center">
          <div
            v-for="tab in ['paying', 'success', 'all']"
            :key="tab"
            class="flex-1"
          >
            <button
              class="w-full text-sm font-500 py-2.5 rounded-xl transition-all"
              :class="[activeTab === tab ? 'bg-[#FF9933] text-white shadow-sm' : 'text-gray-600 hover:bg-gray-50']"
              @click="activeTab = tab as any"
            >
              {{ t(`sellHistory.tabs.${tab}`) }}
            </button>
          </div>
        </div>
        ...
        <div class="pb-20 flex flex-col" style="min-height: calc(100vh - 200px);">
          <div class="flex flex-1 flex-col">
            <van-list
              ...
              :finished="finished"
              :finished-text="sellList.length > 0 ? 'No more data' : ''"
              @load="loadMore"
            >
              ...
              <div v-else-if="!loading" class="flex flex-1 items-center justify-center py-10">
